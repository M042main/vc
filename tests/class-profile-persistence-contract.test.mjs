...
test("stores class metadata while keeping legacy gallery records readable and filterable", async () => {
  ...
  assert.match(firebase, /classId:\s*hasClassMetadata\s*\?[\s\S]{0,100}:\s*null/);
  assert.match(firebase, /className:\s*hasClassMetadata\s*\?[\s\S]{0,120}:\s*null/);
  ...
  assert.match(gallery, /classFilter\s*===\s*["']all["']/);
  assert.match(gallery, /classFilter\s*===\s*["']unclassified["']/);
  assert.match(gallery, /classFilter:\s*effectiveClassFilter/);
  assert.match(gallery, /entries\.map\s*\(/);
